...
def customized_step_with_grad(
    ...
    if score_dict is not None and mask_dict is not None and guidance_scale > 0.0:
        # 多物件模式
        guidance_factor = guidance_scale * (1 - alpha_prod_t) ** (0.5)
        score = 0.0
        for object_name, stdg_score in score_dict.items():
            if object_name not in mask_dict:
                raise ValueError(f"{object_name} not in mask_dict")
            score += stdg_score
            
        # 應用 guidance 到對應區域
        if indices is not None:
            pred_epsilon[indices] = pred_epsilon[indices] - guidance_factor * score
        else:
            pred_epsilon = pred_epsilon - guidance_factor * score
...
def customized_step(
    ...
    if score_dict is not None and mask_dict is not None and guidance_scale > 0.0:
        ...
        # guidance_factor = guidance_scale * (1 - alpha_prod_t) ** (0.5)
        # for object_name, score in score_dict.items(): (two fg mask)
        #     if object_name not in mask_dict:
        #         continue
                
        #     mask = mask_dict.get(object_name, None) # [F, 1, H, W]
        #     latent_mask = F.interpolate(mask.float(), size=(pred_epsilon.shape[3], pred_epsilon.shape[4]), mode='nearest') # [F, 1, H, W]
        #     latent_mask = latent_mask.permute(1, 0, 2, 3).unsqueeze(0)  # [1, 1, F, H, W]
        #     latent_mask = latent_mask.to(pred_epsilon.dtype)
            
        #     # 應用 guidance 到對應區域
        #     if indices is not None:
        #         pred_epsilon[indices] = pred_epsilon[indices] - guidance_factor * score * latent_mask
        #     else:
        #         pred_epsilon = pred_epsilon - guidance_factor * score * latent_mask
...
def compute_temp_loss_with_mask(temp_attn_prob_example, temp_attn_prob_control, weight_each, sam_mask):
    ...
    for name in temp_attn_prob_example.keys():
        ...
        module_attn_loss = calculate_motion_rank_with_mask(attn_prob_example.detach(), attn_prob_control, rank_k = 1, sam_mask = sam_mask)
...
def calculate_motion_rank_with_mask(tensor_ref, tensor_gen, rank_k = 1, sam_mask = None):
    if rank_k ==0:
        loss = torch.tensor(0.0,device = tensor_ref.device)
    elif rank_k >tensor_ref.shape[-1]:
        raise ValueError("the value of rank_k cannot larger than the number of frames")
        ...
        if sam_mask is not None:
            tensor_spatial_size = int(math.sqrt(tensor_ref.shape[0]))
            sam_mask = F.interpolate(sam_mask.float(), size=(tensor_spatial_size, tensor_spatial_size), mode='nearest').bool() # [f, 1, h, w]
            sam_mask = rearrange(sam_mask, 'f c h w -> (h w) c f').unsqueeze(-1) 
            mask = mask & sam_mask 
...
def compute_semantic_loss_with_mask( temp_attn_key_app, temp_attn_key_control, weight_each, sam_mask, block_type="temp"):
    temp_attn_key_loss = []
    for name in temp_attn_key_app.keys():
        ...
        block_name = ".".join(name.split(".")[:2])
        module_attn_loss = calculate_semantic_loss_with_mask(attn_key_app.detach(), attn_key_control, sam_mask, block_type)
...
def calculate_semantic_loss_with_mask(tensor_app, tensor_gen, sam_mask, block_type):
    
    if block_type == "temp":
        tensor_app = tensor_app.permute(1,0,2) # tensor_app.shape = [16, 1024, 640]
        tensor_gen = tensor_gen.permute(1,0,2)
    # [frame, H*W, head*dim]

    assert sam_mask is not None
    tensor_spatial_size = int(math.sqrt(tensor_app.shape[1]))
    sam_mask = F.interpolate(sam_mask.float(), size=(tensor_spatial_size, tensor_spatial_size), mode='nearest').bool() # [f, 1, h, w]
    sam_mask = rearrange(sam_mask, 'f c h w -> f (h w) c ')  # [16, 1, 16, 16] -> [16, 256, 1]

    ref_masked = (tensor_app*sam_mask).sum(dim=1)/(sam_mask.sum(dim=1)+1e-5) 
    gen_masked = (tensor_gen*sam_mask).sum(dim=1)/(sam_mask.sum(dim=1)+1e-5)
    loss = F.mse_loss(ref_masked.detach(), gen_masked)
    return loss
