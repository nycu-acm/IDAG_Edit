...
from IDAG_Edit.utils.convert_from_ckpt import convert_ldm_unet_checkpoint, convert_ldm_clip_checkpoint, convert_ldm_vae_checkpoint
from IDAG_Edit.utils.convert_lora_safetensor_to_diffusers import convert_lora, load_diffusers_lora
...
@torch.no_grad()
def init_prompt_new(prompt, negative_prompt, pipeline):
    uncond_input = pipeline.tokenizer(
        [negative_prompt], padding="max_length", max_length=pipeline.tokenizer.model_max_length,
        return_tensors="pt"
    )
...
def prev_step(model_output: Union[torch.FloatTensor, np.ndarray], timestep: int,
        ...
        alpha_prod_t_prev = ddim_scheduler.alphas_cumprod[
            prev_timestep] if prev_timestep >= 0 else ddim_scheduler.final_alpha_cumprod
...
def get_noise_pred(unet, latents, t, ddim_scheduler, is_forward=True, context=None, cfg_scale=7.5):
        latents_input = torch.cat([latents] * 2)
        assert context is not None
        guidance_scale = 1 if is_forward else cfg_scale 
        noise_pred = unet(latents_input, t, encoder_hidden_states=context)["sample"]
        noise_pred_uncond, noise_prediction_text = noise_pred.chunk(2)
        # noise_pred = noise_pred_uncond + guidance_scale * (noise_prediction_text - noise_pred_uncond)
        noise_pred = noise_prediction_text + guidance_scale * (noise_prediction_text - noise_pred_uncond)
        if is_forward:
...
@torch.no_grad()
def ddim_loop_with_cfg(pipeline, ddim_scheduler, latent, num_inv_steps, prompt, guidance_scale=7.5):
    context = init_prompt(prompt, pipeline)
    uncond_embeddings, cond_embeddings = context.chunk(2)
    # all_latent = [latent]
    all_latent_dict = {} # return a timestep dict
    ...
    for i in tqdm(range(num_inv_steps)):
        step_t = ddim_scheduler.timesteps[len(ddim_scheduler.timesteps) - i - 1]
        # noise_pred = get_noise_pred_single(latent, step_t, cond_embeddings, pipeline.unet)
        noise_pred_text = get_noise_pred_single(latent, step_t, cond_embeddings, pipeline.unet)
        noise_pred_uncond = get_noise_pred_single(latent, step_t, uncond_embeddings, pipeline.unet)
...
def ddim_loop_with_context_with_cfg(pipeline, ddim_scheduler, latent, config): # num_inv_steps, prompt, guidance_scale=7.5
    context = init_prompt(config.inversion_prompt, pipeline)
    uncond_embeddings, cond_embeddings = context.chunk(2)
    #multiframe_NT
    ...
    # all_latent_dict = {} # return a timestep dict
    latent = latent.clone().detach()
    for i in tqdm(range(config.num_inference_step)):
        step_t = ddim_scheduler.timesteps[len(ddim_scheduler.timesteps) - i - 1]
        noise_pred_text = get_noise_pred_single(latent, step_t, cond_embeddings, pipeline.unet)
        noise_pred_uncond = get_noise_pred_single(latent, step_t, uncond_embeddings, pipeline.unet)
        noise_pred = noise_pred_text + config.cfg_scale * (noise_pred_text - noise_pred_uncond)
        latent = next_step(noise_pred, step_t, latent, ddim_scheduler)
        all_latent.append(latent)

    return  all_latent, context # cond_embeddings

@torch.no_grad()
def ddim_loop_with_context(pipeline, ddim_scheduler, latent, config): # config.num_inference_step, config.inversion_prompt
    context = init_prompt_new(config.inversion_prompt, config.negative_prompt, pipeline)
    uncond_embeddings, cond_embeddings = context.chunk(2)
    # multiframe_NT
    if config.multiframe_NT:
        video_length = latent.shape[2]
...
@torch.no_grad()
def ddim_inversion_with_context(pipeline, ddim_scheduler, video_latent, config):
    ddim_latents, context = ddim_loop_with_context(pipeline, ddim_scheduler, video_latent, config)
    return ddim_latents, context

@torch.no_grad()
def ddim_inversion_with_context_with_cfg(pipeline, ddim_scheduler, video_latent, config):
    ddim_latents, context = ddim_loop_with_context_with_cfg(pipeline, ddim_scheduler, video_latent, config)
    return ddim_latents, context
...
def save_video(raw_frames, save_path, fps=10):
    # 确保 raw_frames 是 PyTorch Tensor
...
def save_video_h(raw_frames, save_path, fps=10):
    # 确保 raw_frames 是 PyTorch Tensor
    if isinstance(raw_frames, np.ndarray):
        raw_frames = torch.from_numpy(raw_frames)
    video_codec = "libx264"
    video_options = {
        "crf": "10",  # Constant Rate Factor (lower value = higher quality, 18 is a good balance)
        "preset": "veryslow",  # Encoding preset (e.g., ultrafast, superfast, veryfast, faster, fast, medium, slow, slower, veryslow)
    }
